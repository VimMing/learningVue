...
describe('Observer', function () {
    ...
    describe('.unobserve()', function () {
        ...
        // })
        // 我觉得应该是这样的。
        // 本来obj上有8个listener， convent函数里添加了2个， observe里面添加了6个
        // unobserve里取消了3个， 总共还剩5个， set: 2, mutate: 2, get: 1
        it('should turn off corresponding event listeners', function () {
            var callbacks = obj.__emitter__._cbs
            assert.strictEqual(callbacks['get'].length, 1)
            assert.strictEqual(callbacks['mutate'].length, 2)
            assert.strictEqual(callbacks['set'].length, 2)
        })
